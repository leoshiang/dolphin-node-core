...
class 矩陣 extends Array {
     ...
     * @param {矩陣|number} 對象
     * @return {矩陣} 新矩陣。
     * @throws {型別錯誤} 當參數對象不是矩陣或數值時，會拋出此例外。
     * @example
    ...
    乘(對象) {
        ...
        if (this.#直行數 !== 對象.橫列數) {
            throw new 參數錯誤('矩陣橫列數目應等於對象矩陣直行數目。')
        }
        新矩陣 = new 矩陣(this.橫列數, 對象.直行數)
        for (let 橫列 = 0; 橫列 < this.橫列數; 橫列++) {
            let 新矩陣的橫列 = 新矩陣[橫列]
    ...
    合併(其他矩陣) {
        let 新矩陣 = new 矩陣(this.橫列數, this.直行數 + 其他矩陣.直行數)
        for (let 橫列編號 = 0; 橫列編號 < 新矩陣.橫列數; 橫列編號++) {
            for (let 直行編號 = 0; 直行編號 < this.直行數; 直行編號++) {
                新矩陣[橫列編號][直行編號] = this[橫列編號][直行編號]
            }
            for (let 直行編號 = 0; 直行編號 < 其他矩陣.直行數; 直行編號++) {
                新矩陣[橫列編號][直行編號 + this.直行數] = 其他矩陣[橫列編號][直行編號]
            }
        }
        return 新矩陣
    }
    ...
    相等(M) {
        ...
        for (let 橫列編號 = 0; 橫列編號 < this.#橫列數; 橫列編號++) {
            let 自身橫列快取 = this[橫列編號]
            let M橫列快取 = M[橫列編號]
            for (let 直行編號 = 0; 直行編號 < this.#直行數; 直行編號++) {
                if (!型別.數值相等(自身橫列快取[直行編號], M橫列快取[直行編號])) return false
            }
        }
        return true
    }
    ...
    逆矩陣() {
        ...
        let 暫存矩陣 = this.合併(矩陣.單位矩陣(this.橫列數))
