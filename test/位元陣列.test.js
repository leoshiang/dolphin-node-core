...
describe('測試【取得狀態】', function () {
  test('位元編號不是數值，應拋出型別錯誤例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.取得狀態()).toThrow(型別錯誤)
  })

  test('位元數 24，取得位元編號 -1 的狀態，應拋出索引超出範圍例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.取得狀態(-1)).toThrow(索引超出範圍例外)
  })

  test('位元數 24，取得位元編號 24 的狀態，應拋出索引超出範圍例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.取得狀態(24)).toThrow(索引超出範圍例外)
  })

  test('位元數 24，取得位元編號 25 的狀態，應拋出索引超出範圍例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.取得狀態(25)).toThrow(索引超出範圍例外)
  })
...
describe('測試【設定狀態】', function () {
  ...
  test('位元數 24，設定位元編號 -1 的狀態，應拋出索引超出範圍例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.設定狀態(-1)).toThrow(索引超出範圍例外)
  })

  test('位元數 24，設定位元編號 24 的狀態，應拋出索引超出範圍例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.設定狀態(24)).toThrow(索引超出範圍例外)
  })

  test('位元數 24，設定位元編號 25 的狀態，應拋出索引超出範圍例外', () => {
    const bits = new 位元陣列(24)
    expect(() => bits.設定狀態(25)).toThrow(索引超出範圍例外)
  })
...
describe('測試【設定二進位字串】', function () {
  test('10101010，應能解析為10101010', () => {
    const bits = new 位元陣列(0).設定二進位字串('10101010')
    expect(bits.取得二進位字串()).toBe('10101010')
  })

  test('1010101010101010101010101，應能解析為1010101010101010101010101', () => {
    const bits = new 位元陣列(0).設定二進位字串('1010101010101010101010101')
    expect(bits.取得二進位字串()).toBe('1010101010101010101010101')
  })
})

describe('測試【and】', function () {
  test('如果傳入的陣列長度不一樣，應拋出參數錯誤', () => {
