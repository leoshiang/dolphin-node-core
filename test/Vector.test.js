...
describe('測試【遍歷每個元素但是排除】', function () {
  test('排除的項目不是Bits，應拋出TypeError例外', function () {
    const 五個數字的向量 = new Vector([1, 2, 3, 4, 5])
    expect(() => 五個數字的向量.遍歷每個元素但是排除()).toThrow(TypeError)
  })

  test('[1,2,3,4,5] 排除 [0,4]，回呼函數應收到[2,3,4]', function () {
    const 五個數字的向量 = new Vector([1, 2, 3, 4, 5])
    const 排除項目 = new Bits(5).設定狀態(0, true).設定狀態(4, true)
    let 回呼函數收到的資料 = []
...
describe('測試【修改每個元素但是排除】', function () {
  test('排除的項目不是Bits，應拋出TypeError例外', function () {
    const 五個數字的向量 = new Vector([1, 2, 3, 4, 5])
    expect(() => 五個數字的向量.修改每個元素但是排除()).toThrow(TypeError)
  })
...
describe('測試【最大值】', function () {
  test('效能測試：10000000個元素', function () {
    const 測試向量 = new Vector(10000000)
